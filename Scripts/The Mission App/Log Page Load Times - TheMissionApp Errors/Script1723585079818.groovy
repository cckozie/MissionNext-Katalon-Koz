...
sql.executeInsert('INSERT INTO tma_new_web_pages_bypass (string)' + valueClause)
*/
...
while (!(done)) {
    ...
    if (!(linkList.isEmpty())) {
        ...
        //   Pop link from stack
        //   Retrive the link list from tma_new_web_page_links
        // Else: stack is empty
        //   Set done flag
    } else {
...
//		if (pageStack.size() > 0) {
			...
			sql.execute("update tma_new_web_page_links set processed = true where to_url = '" + toUrl + "'")
...
*/
            sql.eachRow("select to_url from tma_new_web_page_links where processed = false and from_url = '" + fromPage + "'", { def row ->
                    linkList.add(row[0])
                })
//            links = sql.execute("select to_url from tma_new_web_page_links where from_url = '" + fromPage + "'")
...
//insert into tma_new_web_pages_temp (url)
//		WebUI.delay(1)
...
//    WebUI.navigateToUrl(link)
	...
	if(notBlog) {
	    ...
	    if (linkList != null) {
	        sql.execute('delete from tma_new_web_pages_temp')
...
//					println('++++ href is ' + url)
					if(url.indexOf('widgets') < 0 && url.indexOf('javascript') < 0 && url.indexOf('wp-admin') < 0) {
        ...
        if (valueClause != 'value') {
            sql.executeInsert('insert into tma_new_web_pages_temp (url)' + valueClause)
			
            sql.executeInsert("insert into tma_new_all_web_page_links select '" + link + "', url from tma_new_web_pages_temp")
			
//			sql.execute("delete from tma_new_web_pages_temp where url like '%missionnext.org%' or url not like '%explorenext.org%' or url like '%#%'")
//			sql.execute("delete from tma_new_web_pages_temp where url not like '%themissionapp.com%'")
			sql.execute("delete from tma_new_web_pages_temp where url not like 'https://themissionapp.com%' AND url not like 'https://www.themissionapp.com%'")
			
//            sql.execute('delete from tma_new_web_pages_temp a where exists (select 1 from tma_new_web_pages_bypass b where a.url like \'%\' || b.string || \'%themissionapp.com%\')')

            sql.execute('delete from tma_new_web_pages_temp a where exists (select 1 from tma_new_web_page_links b where a.url = b.from_url or a.url = b.to_url)')

            sql.execute(('delete from tma_new_web_pages_temp a where url = \'' + link) + '\'')
			
            sql.executeInsert("insert into tma_new_web_page_links select '" + link + "', url from tma_new_web_pages_temp")
			
            sql.eachRow('select url from tma_new_web_pages_temp', { def row ->
                    pageLinks.add(row[0])
                })
        }
...
def updateDB(def fromUrl, def myUrl, def where, def duration) {
    ...
    sql.query('SELECT COUNT(*) from tma_new_page_load_times ' + toWhere, { def resultSet ->
    ...
    if (rows == 0) {
        sql.executeInsert('INSERT INTO tma_new_page_load_times VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [fromUrl, myUrl, duration
                , duration, duration, 1, now, now, now])
    } else {
        sql.query('SELECT * from tma_new_page_load_times ' + toWhere, { def resultSet ->
                while (resultSet.next()) {
        ...
        sql.executeUpdate("UPDATE tma_new_page_load_times SET max_time_from_url = $maxUrl, minimum_time = $min, maximum_time = $max, average_time = $newAvg, test_count = $count,			minimum_timestamp = $minTimeStamp, maximum_timestamp = $maxTimeStamp, last_timestamp = $lastTimeStamp " + 
            toWhere)
    }
    
    sql.executeInsert('INSERT INTO tma_new_web_pages VALUES (?)', [myUrl])
}

def clearTables() {
    println('******************* Clearing tables ***********************')

    sql.execute('DELETE FROM tma_new_web_pages_bypass')

    sql.execute('DELETE FROM tma_new_web_pages')

    sql.execute('DELETE FROM tma_new_web_page_errors')

    sql.execute('DELETE FROM tma_new_web_page_links')

    sql.execute('DELETE FROM tma_new_all_web_page_links')

    sql.execute('DELETE FROM tma_new_page_load_times')
