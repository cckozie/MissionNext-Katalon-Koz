...
ttPath = '/Users/cckozie/git/MissionNext-Katalon-Koz/images/education candidate/'
...
tooltipTextMap = WebUI.callTestCase(findTestCase('_Functions/Get Screenshot and Tooltip Text'), [('varExtension') : 'Register'], FailureHandling.STOP_ON_FAILURE)
...
//tooltipTextMap = WebUI.callTestCase(findTestCase('_Functions/Get Tooltip Text'), [('varFormat') : 'map'], FailureHandling.STOP_ON_FAILURE)
...
for (it in tooltipText) {
	...
	if(actualText != myText) {
		outText = '####### ERROR: The tooltip text for ' + myKey + ' should be ' + myText + ' but instead is ' + actualText + '.'
		println(outText)
...
for (def it : tooltips) {
    ...
    tObj = ('Education Candidate Profile/Education Register/' + myValue)
