...
testName = 'Education Candidate Spouse Teaching Prefs Tab'
...
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
parms = [varSpouse_preferred_education_positions]

//xpath of the Positions group
positions = "//input[@id='profile_group-1631421746.766_spouse_preferred_education_positions']"
...
if(!GlobalVariable.fastPath) {
	...
	object = 'Education Candidate Profile/Tabs/Spouse Teaching Prefs/input_Administrator'
	WebUI.callTestCase(findTestCase('_Functions/Perform Action'), [('varAction'): 'click', ('varObject') : object], FailureHandling.STOP_ON_FAILURE)
}


if (varSpouse_education_preferences_comment != null) {
	object = 'Object Repository/Education Candidate Profile/Tabs/Spouse Teaching Prefs/textarea_Spouse Education Preferences Comment'
	WebUI.callTestCase(findTestCase('_Functions/Perform Action'), [('varAction'): 'setText',
		('varObject') : object, ('varParm1') : varSpouse_education_preferences_comment], FailureHandling.STOP_ON_FAILURE)
}
...
myClass = WebUI.getAttribute(findTestObject('Education Candidate Profile/Tabs/a_Spouse Teaching Prefs'), 'class', FailureHandling.OPTIONAL)
