...
profiles = [
	...
	'Journey Guide 01' : ['My Profile', 'My Dashboard', 'Logout']
	]
...
//				driver.findElement(By.xpath("//input[@type='checkbox' and @value='Elementary School Teacher']")).click();
			...
			List<WebElement> buttons = driver.findElements(By.xpath("//input[@type='button' and @value='Matches']"));
...
//			WebUI.click(findTestObject('Object Repository/Timings/button_Matches'))
		...
		} else if(link == 'My Dashboard' && site == 'Journey Guide') {
			
			action = "Click Candidate Dashboard"
			
			timeStart = new Date()
			
//			WebUI.click(findTestObject('Timings/a_Dashboard Link Parm', [('link') : link]))
			
			WebUI.click(findTestObject('Object Repository/Timings/a_Candidate Dashboard'))
						
			WebUI.waitForPageLoad(60)
			
			updateDB()
			
			action = "Click Candidate Name Search"
			
			timeStart = new Date()
			
			WebUI.click(findTestObject('Object Repository/Timings/button_Candidate Name Search'))
			...
			action = "Return to Journey Guide Dashboard"
...
//	String where = 'WHERE from_url = \'' + from + '\' AND to_url = \'' + to + '\''
	...
	query = 'SELECT COUNT(*) from bow_mn_page_load_times ' + where
	...
	if (rows == 0) {
		sql.executeInsert('INSERT INTO bow_mn_page_load_times VALUES (?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [host, from, to, duration, duration
				, duration, 1, now, now, now, from, action, user])
		
	} else {
		query = 'SELECT * from bow_mn_page_load_times ' + where
		println(query)
		sql.query(query, { def resultSet ->
				while (resultSet.next()) {
...
//		minTimeStamp = ''
		...
		sql.executeUpdate("UPDATE bow_mn_page_load_times SET server = $host, max_time_from_url = $maxUrl, minimum_time = $min, maximum_time = $max, average_time = $newAvg, test_count = $count, minimum_timestamp = $minTimeStamp, maximum_timestamp = $maxTimeStamp, last_timestamp = $lastTimeStamp, last_action = $action, user_profile = $user " + where)
