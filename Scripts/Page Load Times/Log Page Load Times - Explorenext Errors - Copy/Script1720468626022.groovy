...
def navigateToUrl(def from, def link) {
	...
	if(linkList != null) {
		...
		sql.execute("delete from en_web_pages_temp a where exists (select 1 from mn_web_pages_bypass b where a.url like '%' || b.string || '%')")
...
def updateDB(def fromUrl, def myUrl, def where, def duration) {
	...
	sql.query('SELECT COUNT(*) from page_load_times ' + toWhere, { def resultSet ->
	...
	if (rows == 0) {
		...
		sql.query('SELECT * from page_load_times ' + toWhere, { def resultSet ->
				while (resultSet.next()) {
