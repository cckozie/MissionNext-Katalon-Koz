...
while (!(done)) {
    ...
    if (myKey != null) {
        ...
        println('myList is ' + myList) //				if(myKey == lastPushed) {			
        //					myKey = keyStack.pop()
        //					outFile.append('popped,' + myKey + '\n')
        //				}
    } //	} else {
    //		WebUI.delay(60)
    //	}
...
def getLinkList(def fromPage, def url) {
    println('========= from page ' + fromPage)

    fromPage = fromPage.replace('http:', 'https:')
...
def writeLinks(def from, List toList) {
    from = from.replace('http:', 'https:')

    for (def toUrl : toList) {
        toUrl = toUrl.replaceAll('http:', 'https:')
...
def navigate(def from, def link) {
    ...
    if (rows == 0) {
        ...
        sql.executeUpdate("UPDATE page_load_times SET minimum_time = $min, maximum_time = $max, average_time = $newAvg, test_count = $count,			minimum_timestamp = $minTimeStamp, maximum_timestamp = $maxTimeStamp, last_timestamp = $lastTimeStamp " + 
            whereClause)
    }
}
