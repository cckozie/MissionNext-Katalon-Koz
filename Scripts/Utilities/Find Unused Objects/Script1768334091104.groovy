...
if(list.size() > 0) {
	list.each {
		if (it.path.contains('.rs')) {
			...
			start = 'Object Repository/'
			base = obj.indexOf(start) + start.length()
			/*
			loc = obj.lastIndexOf('/')
			...
			objects.add(obj)
			*/
			objects.add(obj.substring(base))
		}
	}
}
//println(objects)
...
folders.each {
	...
	list1.each {
		if (it.path.contains('groovy')) {
			...
			objects.each {
				if (text.contains(it) || text.contains(start + it) && usedObjects.indexOf(it) < 0) {
					usedObjects.add(it)
				}
			}
		}
