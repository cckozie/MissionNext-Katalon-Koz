...
for (def field : inputFields) {
	...
	if(myName.contains('time_commitment')) {
		...
		println('myValue is ' + myValue + ' and myType is ' +field.getAttribute('type'))
		if(myValue.contains('!')) {
			println(myName + ' wildcard value is ' + myValue + '\n\n')
			WebElement label = field.findElement(By.xpath("following-sibling::*"));
			println('label is ' + label.getAttribute('innerHTML'))
		}
	}
}
