...
println('There are now ' + handles.size() + ' windows.')
...
adWindow = driver.getWindowHandle();

title = adWindow.getWebDriver().getTitle()

println('On return from get match rules on AD window index is ' + WebUI.getWindowIndex())

println('There are now ' + driver.getWindowHandles().size() + ' windows. Current window title is ' + title)

System.exit(0)

println('On return from rules page window index is ' + WebUI.getWindowIndex())
...
println('There are now ' + driver.getWindowHandles().size() + ' windows.')

rulesWindow = driver.getWindowHandle();


//WebUI.closeWindowIndex(rulesWindow)

//WebUI.switchToWindowIndex(adWindow)
...
emptyWindow = driver.getWindowHandle();
...
println('After logging in as organization, there are now ' + driver.getWindowHandles().size() + ' windows.')

orgWindow = driver.getWindowHandle();

WebUI.click(findTestObject('Object Repository/Education Partner Profile/Dashboard/a_Educator Matches'))

println('Now on match table page. There are now ' + driver.getWindowHandles().size() + ' windows.')

tableWindow = driver.getWindowHandle();
...
def doMatching(candidateSelections, organizationSelections) {
	...
	outText = (('Match percentage is ' + points.round(1)) + '%.')
