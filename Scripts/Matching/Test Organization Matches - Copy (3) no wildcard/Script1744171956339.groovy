...
candidate = '1' // Set candidate number from csv file, or '0' for all candidate
...
errorFile.write('Running ' + myTestCase + ' logging errors.\n')
				...
				if(!excluded) {
					...
					if(!matched) {
						...
						for(userValue in userValues) {
							if(!matched) {
								println('test for wildcard')
								for(val in orgValues) {
									println(val)
								...
								if(userValue in orgValues || matched) {
