...
if (!(updateWildcards)) {
    ...
    partnerWildcards = [('Time Commitments') : ['Open - Will negotiate'], ('Available Start Options') : ['No Preference']
        , ('Time/Hours Needed') : ['Open'], ('Short-Term Trip Lengths') : ['Open'], ('Short-Term Availability') : ['Open']
        , ('Short-Term Objective') : ['Any or all candidate objectives'], ('Preferred Region(s)') : ['No Preference'], ('Process Stage') : [
            'I am just beginning to look at missions'], ('Cross-cultural Experience') : ['Not served in a culture other than my own']
        , ('Bible Training') : ['Not Applicable'], ('Attended Perspectives?') : ['I have not taken the Perspectives Course']
        , ('Relocation Option(s)') : ['Not a Match criterion'], ('Paid & Volunteer Positions') : ['No Preference'], ('Profile Years') : [
            'All'], ('Affiliated with a Church?') : ['No']]

    //Education Candidate Wildcards
    candidateWildcards = [('Time Commitment(s)') : ['Open - Will negotiate'], ('Time/Hours Available') : ['Open'], ('Paid & Volunteer Positions') : [
            'Volunteer/self-supported position', 'Open'], ('Length of Assignment') : ['Open'], ('Short-Term Availability') : [
            'Open'], ('Travel Options') : ['No travel funding provided'], ('Preferred Region(s)') : ['No Preference'], ('I/We can be Available') : [
            'Not sure'], ('Relocation Option(s)') : ['Not sure']] //Load new wildcard maps for candidates and partners
} else {
...
while (pageCount <= pages) {
    ...
    for (def it : iconLocs) {
        ...
        s.wait(imagePath + 'Blue Bar.png', 10)

        if (s.exists(imagePath + 'Candidate Profile Close Button.png')) {
            s.hover(imagePath + 'Candidate Profile Close Button.png')

            button = 'small'

            s.wheel(Mouse.WHEEL_UP, 5)
        } else {
            s.hover(imagePath + 'Candidate Profile Big Close Button.png')
        ...
        regMaritalStatus = s.find(imagePath + 'Marital status')
...
//Need to allow for cWC being a list
    ...
    for (def it : matchValues) {
        ...
        if (!(excluded)) {
            ...
            if ((cWC != null) && (candidateValues != null)) {
				outFile.append('In if 1\n')
				outFile.append('Match is ' + match + '\n')
                if (cWC.intersect(candidateValues).size() > 0) {
					outFile.append('In if 2\n')
			...
			if (!(match)) {
                ...
                if ((oWC != null) && (organizationValues != null)) {
					outFile.append('In if 3\n')
					
                    if (oWC.intersect(organizationValues).size() > 0) {
						outFile.append('In if 4\n')
				...
				if ((!(match) && (candidateValues != null)) && (organizationValues != null)) {
					outFile.append('In else 2, if 5\n')
					outFile.append('Testing intersection')
                    matches = candidateValues.intersect(organizationValues)
					outFile.append('Matches.size() is ' + matches.size())
                    if (matches.size() > 0) {
						outFile.append('In if 6\n')
						
						outFile.append('Size is greater')
