	...
	for (def it : matchValues) {
			...
			if (!(match)) {
				...
				if ((jWC != null) && (jobValues != null)) {
					...
					if (jWC.intersect(jobValues).size() > 0) {
						...
						outText = (((('Job wildcard match on ' + jWC.intersect(jobValues)) + '. Adding ') +
						newPoints) + ' points.')
				...
				if ((!(match) && (candidateValues != null)) && (jobValues != null)) {
					...
					if (matches.size() > 0) {
						outText = (((('Found match on ' + candidateValues.intersect(jobValues)) + '. Adding ') +
						newPoints) + ' points.')
		...
		if (match) {
			...
			if (matchValue == 5) {
				...
				if(!category || jobValues != null) { //Ignore no match if it's a category and job has no selection for it
					excluded = true
