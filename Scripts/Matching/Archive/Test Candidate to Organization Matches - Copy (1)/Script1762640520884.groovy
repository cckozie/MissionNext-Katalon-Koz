...
if (site == 'Journey') {	
	...
	candidateFieldValues = WebUI.callTestCase(findTestCase('Matching/_Functions/Get Profile from API'), [('varEmail') : candidateEmail, ('varSite') : site], 
	    FailureHandling.STOP_ON_FAILURE)
	candidateFieldValues.each {
		println(it)
	}

	for(it in candidateFieldValues){ 
	...
	candidateSelections.put('Your Country of Citizenship',country)
