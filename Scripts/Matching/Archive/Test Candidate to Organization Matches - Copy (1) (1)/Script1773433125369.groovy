    ...
    for (row = 1; row < row_count; row++) {
		...
		if(rowClass.contains('item')) {
	        ...
	        if (site == 'Journey') {
	            ...
	            userFound = WebUI.callTestCase(findTestCase('Matching/_Functions/Get Profile from AD'), [('varEmail') : email, ('varFile') : profileFile
	                    , ('varSearchType') : 'email'], FailureHandling.STOP_ON_FAILURE)
				
				if(!userFound) {
					outText = '\n----- User email ' + email + ' was not found. Bypassing organization'
					println(outText)
					outFile.append(outText + '\n')
...
def getNextLine(def reader, def categories) {
    ...
    if (((line != null) && !(line.contains('Logout'))) && !(line.contains('Note: A candidate user'))) {
		if(line.contains('\t')) {
			println('+++There is a tab in the line')
		} else {
			println('---There is NO tab in the line')
		}
		println('***The tab is at position ' + line.indexOf('\t'))
        return line
    } else {
        return 'end'
    }
...
def formatProfile(def file, def categories, def matchFields) {
    ...
    while (line != 'end') {
		println('while 1')
        line = getNextLine(reader, categories)	//2
        ...
        while (line != 'end') {
            ...
            if ((line.length() > 3) && line.substring(1, 4).contains('\t')) {
                ...
                lineTab = line.indexOf('\t')
				println('1-tab is at position ' + lineTab)

                number = line.substring(0, lineTab)
                ...
                if (!(fieldNumbers.contains(number))) {
                   ...
                   fieldNumbers.add(number)
                    ...
                    line = line.substring(lineTab + 1)

                    lineTab = line.indexOf('\t')
					println('2-tab is at position ' + lineTab)
