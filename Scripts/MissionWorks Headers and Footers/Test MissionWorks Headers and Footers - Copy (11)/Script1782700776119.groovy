...
//		WebUI.waitForPageLoad(30)
		...
		if(first) {
			...
			if(myObjectFolder != null) {
				...
				arrowClickable = WebUI.verifyElementClickable(findTestObject(myObjectFolder + arrow), FailureHandling.OPTIONAL)
			...
			} else if(site == 'MissionLinked') {//arrow not found, hovering over MW logo if site is MissionLinked
				
				WebUI.delay(2)
				
				logo = 'MissionWorks Headers and Footers/Header/img_MissionWorks logo'
				...
				if(myObjectFolder != null) {
					...
					exists = testForObjectExists(folderBase + '/', arrow)
...
//	WebElement connecting = driver.findElement(By.xpath("//*[text()='Connecting']"))
//	WebElement element = driver.findElement(By.xpath("//*[contains(text(), 'Connecting')]"))
	List<WebElement> connectingElements = driver.findElements(By.xpath("//a[contains(text(), 'Connecting')]"))
	
	if(connectingElements.size() > 0) {
		
		connectingElements[0].click()
		
	} else { 
		outFile.append('#### ERROR: Unable to find the Connecting menu in the header."\n')
		errorFlag = true
	}
