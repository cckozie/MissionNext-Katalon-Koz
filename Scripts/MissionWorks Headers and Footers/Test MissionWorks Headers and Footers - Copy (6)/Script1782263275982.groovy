...
def testForHeaderFooter(testObjects, folderBase, testFolders, testText) {
...
//	testObjectFolders = ['/', '/Custom1/']
	testObjectFolders = testFolders
	
	for(folder in testObjectFolders) {
		
		myFolder = folderBase + folder
		
		myTestObject = myFolder +  testObject
		
		println(myTestObject)
		...
		try {
			...
			if(testText != null) {
				...
				elementText = WebUI.getText(findTestObject(myTestObject)).toLowerCase()
				
				println(elementText)
				
				if(elementText.contains(testText)) {
					
					retCode = folder
					
					break
				} else {
					force = 1/0	//force exception is element found but text is wrong
				}
			} else {
	...
	for(option in menuOptions) {
		...
		if(headerFolder == null) {
			...
			if(folder == null) {
				...
				exists = WebUI.verifyElementPresent(findTestObject('MissionWorks Headers and Footers/Header/img_MissionWorks logo'), 1, FailureHandling.OPTIONAL)
				
				println('>>>>> exists is ' + exists)
				
				if(exists) {
				
					WebUI.mouseOver(findTestObject('MissionWorks Headers and Footers/Header/img_MissionWorks logo'), FailureHandling.OPTIONAL)
					...
					println('---- Could not find the MissionWorks logo.')
				}
				//System.exit(0)
			}
		}
		...
		WebUI.navigateToUrl(url)
			...
			outFile.append('#### ERROR: Unable find down-arrow on Connecting menu. Bypassing test on the header.\n')
			errorFlag = true
