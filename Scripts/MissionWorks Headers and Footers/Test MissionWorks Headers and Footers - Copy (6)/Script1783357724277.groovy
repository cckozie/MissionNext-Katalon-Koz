...
//			WebUI.delay(1)			
		...
		if(first) {
			...
			if(screensize != 'full') {
				...
				folder = getObjectFolder(hamburgerBase, hamburger)
	...
	//			WebUI.delay(2)
		...
		if(arrow != null) {
			...
			if(option != 'About Us' || optionCount > 0) {
...
//						System.exit(0)
				...
				if(screensize != 'full') {
					...
					if(arrowClickable) {
						...
						try {
							...
							outFile.append('#### ERROR: Unable to click on the About Us header.\n')
							errorFlag = true
						}
				...
				if(optionclickable) {
					...
					println('>>> clicking header ' + option)
					...
					outFile.append('#### ERROR: Unable to click on the About Us option.\n')
					errorFlag = true
				}
			}
