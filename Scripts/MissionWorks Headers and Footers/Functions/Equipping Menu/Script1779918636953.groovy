...
menuOptions = ['Coaching', 'MissionLinked','MissionArmor', 'MissionExcellence']
...
if(outFile == '') {
	outFile = new File('/Users/cckozie/Documents/MissionNext/Test Reports/Testing Equipping Menu Options.txt')
	outFile.write('     Testing Equipping Menu Options\n')
}
...
outFile.append('\n<<< TESTING THE EQUIPPING MENU ON ' + site + ' >>>\n')
...
for(option in menuOptions) {
	...
	outFile.append('\nTesting ' + option + ' link on the Equipping menu on ' + site + '.\n')
	
//	println('\nTesting ' + option + ' option on the Equipping menu.\n')
	println('>>> clicking sub-arrow')
	
	arrowClickable = WebUI.waitForElementClickable(findTestObject(objectFolder + 'span_Equipping sub-arrow'), 1)
	
	if(arrowClickable) {
		
		println('>>> clicking menu option ' + option)
	
		WebUI.click(findTestObject(objectFolder + 'span_Equipping sub-arrow'))
