...
def navigateToUrl(def from, def link) {
	...
	if(notBlog) {
        ...
        if (valueClause != 'value') {
            ...
            sql.executeInsert("insert into mn_web_page_errors select '" + link + "', url from mn_web_pages_temp where url like '%explorenext.org%'")

//			sql.execute("delete from mn_web_pages_temp where url like '%missionnext.org%' or url not like '%explorenext.org%' or url like '%#%'")
