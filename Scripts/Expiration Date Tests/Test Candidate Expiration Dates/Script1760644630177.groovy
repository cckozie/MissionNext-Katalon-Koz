...
subscriptions = WebUI.callTestCase(findTestCase('_Functions/Get Candidate Subscription Data'), [:], FailureHandling.STOP_ON_FAILURE)
...
for (def site : sites) {
    ...
    WebUI.selectOptionByValue(findTestObject('Object Repository/Admin/Ad User Viewer Utility/select_Role'), 'Candidate', 
        false)
    ...
    for (def subscription : subscriptions) {
        ...
        subValues = subscription.value

        println(subKey)

        println(subValues)

        values = candidates.get(subKey, null)

        if (values != null) {
            println(values)

            lastLogin = (subValues[1]).substring(0, 10)

            endDate = (subValues[2]).substring(0, 10)
            ...
            if ((status == 'ERROR') || !(errorsOnly)) {
                ...
                outFile.append(((((((((((((((((subKey + ',"') + user) + '","') + email) + '","') + firstName) + '","') + 
                    lastName) + '","') + lastLogin) + '","') + endDate) + '","') + expires) + '","') + status) + '"\n')
            }
        }
    }
