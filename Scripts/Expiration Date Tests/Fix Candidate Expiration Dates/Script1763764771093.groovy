import static com.kms.katalon.core.checkpoint.CheckpointFactory.findCheckpoint
import static com.kms.katalon.core.testcase.TestCaseFactory.findTestCase
import static com.kms.katalon.core.testdata.TestDataFactory.findTestData
import static com.kms.katalon.core.testobject.ObjectRepository.findTestObject
import static com.kms.katalon.core.testobject.ObjectRepository.findWindowsObject
import com.kms.katalon.core.checkpoint.Checkpoint as Checkpoint
import com.kms.katalon.core.cucumber.keyword.CucumberBuiltinKeywords as CucumberKW
import com.kms.katalon.core.mobile.keyword.MobileBuiltInKeywords as Mobile
import com.kms.katalon.core.model.FailureHandling as FailureHandling
import com.kms.katalon.core.testcase.TestCase as TestCase
import com.kms.katalon.core.testdata.TestData as TestData
import com.kms.katalon.core.testng.keyword.TestNGBuiltinKeywords as TestNGKW
import com.kms.katalon.core.testobject.TestObject as TestObject
import com.kms.katalon.core.webservice.keyword.WSBuiltInKeywords as WS
import com.kms.katalon.core.webui.keyword.WebUiBuiltInKeywords as WebUI
import com.kms.katalon.core.windows.keyword.WindowsBuiltinKeywords as Windows
import internal.GlobalVariable as GlobalVariable
import org.openqa.selenium.Keys as Keys
import com.kms.katalon.core.configuration.RunConfiguration as RunConfiguration
import org.openqa.selenium.By as By
import org.openqa.selenium.WebDriver as WebDriver
import org.openqa.selenium.WebElement as WebElement
import com.kms.katalon.core.webui.driver.DriverFactory as DriverFactory
import java.io.File as File

// FIX SO THAT TEST AND EXPIRED ACCOUNTS GET PROCESSED AND 'NONE' SECTION GETS IGNORED

// I SEE DIFFERENT END DATES ON THE UNFILTERED SUBSCRIPTION PAGE THAT SHOWS ALL CANDIDATES (https://ad.missionnext.org/subscriptions.php) AND THE PAGE THAT SHOWS 
// ONLY A SINGLE SITE. DO THEY GET RECONCILED OVERNIGHT?


site = 'QuickStart'

printOnly = false

maxEdits = 100

editCount = 0

varFormat = "YY-MM-dd_HH-mm"

now = new Date().format(varFormat)


myTestCase = RunConfiguration.getExecutionSource().toString().substring(RunConfiguration.getExecutionSource().toString().lastIndexOf(
        '/') + 1)

myTestCase = myTestCase.substring(0, myTestCase.length() - 3)

filePath = '/Users/cckozie/git/MissionNext-Katalon-Koz/Data Files/'

//outFile = new File(('/Users/cckozie/Documents/MissionNext/Test Reports/' + myTestCase) + '.txt')

dataFile = new File('/Users/cckozie/Documents/MissionNext/Test Data/' + myTestCase + '-' + site + '_List_' + now + '.csv')

editsFile = new File('/Users/cckozie/Documents/MissionNext/Test Data/' + myTestCase + '-' + site + '_Edits_' + now + '.csv')

imagePath = '/Users/cckozie/Documents/MissionNext/Test Data/Subscription Edit Screenshots/'



//GlobalVariable.outFile = outFile
//outFile.write(('Running ' + myTestCase) + '\n\n')
if (!(printOnly)) {
    if (!(editsFile.isFile())) {
        editsFile.write(('Running ' + myTestCase) + '\n\nChanges made to candidate subscription expiration dates.\n\n')

        editsFile.append('line,id,days,last,first,username,last in,expires,changed to,apps')

        editsFile.append('\n')
    }
} else {
    dataFile.write(('Running ' + myTestCase) + '\n\n')

    dataFile.append('line,id,days,last,first,username,last in,expires,should be,apps')

    dataFile.append('\n')
}

WebUI.callTestCase(findTestCase('_Functions/Log In to AD'), [:], FailureHandling.STOP_ON_FAILURE)

WebUI.click(findTestObject('Object Repository/Admin/Ad Subscription Utility/a_Subscription Information  Administration'))

WebUI.waitForPageLoad(10)

WebUI.selectOptionByLabel(findTestObject('Object Repository/Admin/Ad Subscription Utility/select_Website'), site, false)

WebUI.selectOptionByLabel(findTestObject('Object Repository/Admin/Ad Subscription Utility/select_Role'), 'Candidate', false)

WebUI.delay(1)

WebUI.click(findTestObject('Admin/Ad Subscription Utility/btn_Add Subscription Entry'))

WebUI.waitForPageLoad(60)

myXpath = '/html/body/center/table/tbody/tr[2]/td[2]/table/tbody/tr[3]/td/table/tbody/tr/td[3]/table/tbody/tr/td[2]/span/div/form/p[3]/table/tbody'

WebDriver driver = DriverFactory.getWebDriver()

WebElement Table = driver.findElement(By.xpath(myXpath))

List<WebElement> Rows = Table.findElements(By.tagName('tr'))

int row_count = Rows.size()

println(row_count + ' rows found')

mmdd = '-12-31'

bypass = []

//row_count = 30
for (row = 1; row < row_count; row++) {
    //for (row = 1; row < 36; row++) {
    List<WebElement> Columns = Rows.get(row).findElements(By.tagName('td'))

    lineNum = Columns.get(0).getText().trim()

    if (lineNum.contains('Account')) {
        outText = '\n'+ lineNum + '\n'
		if(!printOnly) {
			editsFile.append(outText)
		} else {
			dataFile.append(outText)
		}
		
		continue
    }
    
    if (lineNum.contains('NONE')) {
		continue
	}
	
    id = Columns.get(1).getText().trim()
	
	if(id in bypass) {
		continue
	}

    days = Columns.get(2).getText().trim()

    lastName = Columns.get(3).getText().trim()

    lastName = lastName.replace(',', '')

    firstName = Columns.get(4).getText().trim()

    firstName = firstName.replace(',', '')

    apps = Columns.get(6).getText()

    myApps = apps.replace(',', '')

    lastIn = Columns.get(7).getText().trim()

    expireDate = Columns.get(8).getText().trim()

    username = Columns.get(10).getText().trim()

    //    println((((((((((((((((lineNum + ', ') + id) + ', ') + days) + ', ') + lastName) + ', ') + username) + ', ') + firstName) + 
    //        ', ') + lastIn) + ', ') + expireDate) + ', ') + myApps)
    int loginYear = lastIn as int

    int expireYear = ((expireDate.substring(0, 4)) as int)

    int endYear = loginYear + 5

    md = expireDate.substring(4, 10)

    //    println('md is ' + md)
    if ((loginYear != 2019) && ((expireYear != endYear) || (md != mmdd))) {
        println(loginYear)

        newDate = (endYear + mmdd)

        //        println('Expire date should be ' + newDate)
        if (!(printOnly)) {
            Columns.get(5).click()

            WebUI.waitForPageLoad(60)
			
            editXpath = '/html/body/center/table/tbody/tr[2]/td[2]/table/tbody/tr[3]/td/table/tbody/tr/td[3]/table/tbody/tr/td[2]/span/div/form/table/tbody'

            WebElement editTable = driver.findElement(By.xpath(editXpath))

            List<WebElement> editRows = editTable.findElements(By.tagName('tr'))

            int edit_row_count = editRows.size()

            println(edit_row_count + ' edit rows found')

            editRowCount = (edit_row_count - 1)

            for (editRow = 1; editRow < edit_row_count; editRow++) {
                List<WebElement> editColumns = editRows.get(editRow).findElements(By.tagName('a'))

                println(editColumns.size() + ' <a> tags found')

                editLink = editColumns.get(0).getAttribute('href')

                println('edit link is ' + editLink)

                editColumns.get(0).click()

                WebUI.delay(1)
				
				WebUI.takeFullPageScreenshot(imagePath + username + '-' + editRow + '_before.png')

                WebUI.setText(findTestObject('Admin/Ad Subscription Utility/input_YYYY-MM-DD_end_date'), newDate)

                WebUI.delay(1)

                WebUI.click(findTestObject('Object Repository/Admin/Ad Subscription Utility/btn_Add Subscription Entry'))
				
				WebUI.delay(1)
				
				successful = WebUI.verifyTextPresent('Record is successfully updated.', false, FailureHandling.CONTINUE_ON_FAILURE)
					
				if(!successful) {
					
					updateError = WebUI.verifyTextPresent('UPDATE subscriptions SET app_id', false, FailureHandling.CONTINUE_ON_FAILURE)
					
					if(!updateError) {
					
						WebUI.click(findTestObject('Object Repository/Admin/Ad Subscription Utility/btn_Add Subscription Entry'))
			
						WebUI.delay(1)
					
					} else { 
					
						println('ERROR: UPDATE WAS NOT SUCCESSFUL')
						
						editsFile.append(lineNum + ', ' + id + ', ' + days + ', ' + lastName + ', ' + firstName +
							', ' + username + ', ' + lastIn + ', ' + expireDate + ', ' + '*******' + ', ' + myApps + ', ##### UPDATE FAILED')
			
						editsFile.append('\n')
						
						bypass.add(id)
			
//						WebUI.back()
					}
				}

				WebUI.takeFullPageScreenshot(imagePath + username + '_after.png')

                WebUI.back()

                WebUI.delay(1)

                WebUI.back()

                WebUI.delay(1)
				
				

                if (editRow < editRowCount) {
					
					isPresent = driver.findElements(By.xpath(editXpath)).size() > 0
					
					waitTime = 0
					
					while(!isPresent && waitTime < 10) {
						
						WebUI.delay(1)
						
						isPresent = driver.findElements(By.xpath(editXpath)).size() > 0
						
						waitTime++
						
					}
					
                    editTable = driver.findElement(By.xpath(editXpath))

                    editRows = editTable.findElements(By.tagName('tr'))
                }
            }
            
            editsFile.append((((((((((((((((((lineNum + ', ') + id) + ', ') + days) + ', ') + lastName) + ', ') + firstName) + 
                ', ') + username) + ', ') + lastIn) + ', ') + expireDate) + ', ') + newDate) + ', ') + myApps)

            editsFile.append('\n')

            WebUI.back()

            WebUI.delay(1)

            WebUI.refresh()

            editCount++

            if (editCount >= maxEdits) {
                break
            }
            
            WebUI.waitForPageLoad(60)

            topPage = WebUI.verifyElementPresent(findTestObject('Object Repository/Admin/Ad Subscription Utility/select_Role'), 2,
                FailureHandling.CONTINUE_ON_FAILURE)
			
            if (topPage) {
                //Sometimes 'back' goes too far
                WebUI.selectOptionByLabel(findTestObject('Object Repository/Admin/Ad Subscription Utility/select_Role'), 
                    'Candidate', false)

                WebUI.delay(1)

                WebUI.click(findTestObject('Admin/Ad Subscription Utility/btn_Add Subscription Entry'))

                WebUI.waitForPageLoad(60)

                row-- //backup the row in case the user got moved to 'expired' section
            }
            
			isPresent = driver.findElements(By.xpath(myXpath)).size() > 0
			
			waitTime = 0
			
			while(!isPresent && waitTime < 10) {
				
				WebUI.delay(1)
				
				isPresent = driver.findElements(By.xpath(myXpath)).size() > 0
				
				waitTime++
				
			}

            Table = driver.findElement(By.xpath(myXpath))

            Rows = Table.findElements(By.tagName('tr'))
        }
    } else {
        newDate = ''
    }
    
    if (printOnly) {
        dataFile.append((((((((((((((((((lineNum + ', ') + id) + ', ') + days) + ', ') + lastName) + ', ') + firstName) + 
            ', ') + username) + ', ') + lastIn) + ', ') + expireDate) + ', ') + newDate) + ', ') + myApps)

        dataFile.append('\n')
    } else {
    }
}

/*
 * Date Today: 2025-11-21 | Subscription Ends: 2027-12-31 | Grace Period Ends: 2028-01-10$sql_upd query failed:
 * UPDATE subscriptions SET app_id = , partnership = '', price = 0, is_recurrent = 't', start_date = '2021-11-26 23:30:00', end_date = '2027-12-31 23:30:00', status = 'active', comment = '', paid = 0 WHERE id = 21521
*/