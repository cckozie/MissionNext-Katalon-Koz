...
fieldMessages.each({
	...
	WebUI.verifyElementVisible(findTestObject(tObj))
	
	msg = WebUI.getText(findTestObject(tObj))
	...
	if(myObj == 'div_Email message') {
		...
		WebUI.setText(findTestObject(tObj), GlobalVariable.username)
		
		tObj = 'Journey Partner Profile/Register/input_Password'
		
		scrollToObject(tObj)
		
		WebUI.setEncryptedText(findTestObject(tObj), GlobalVariable.password)
		
		tObj = 'Journey Partner Profile/Register/input_Key Contact Email'
		
		scrollToObject(tObj)
		
		WebUI.setText(findTestObject(tObj), GlobalVariable.email)
