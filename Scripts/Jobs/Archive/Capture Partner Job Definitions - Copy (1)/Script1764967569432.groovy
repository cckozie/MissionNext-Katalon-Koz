...
username = 'joshua.r@jesusfilm.org'
...
//			driver.findElement(By.linkText(jobCategory)).click();
			...
			WebElement link = driver.findElement(By.linkText(jobCategory))
			...
			object = 'Object Repository/Journey Partner Profile/New Jobs/button_Edit Job from View Job'
			...
			WebUI.click(findTestObject('Object Repository/Journey Partner Profile/New Jobs/a_Other Criteria'))

			WebUI.waitForPageLoad(60)
			
			prefsXpath = "//input[@id='job_group-1613449251.023_job_preferences']"
			
			ministryPrefs = WebUI.callTestCase(findTestCase('_Functions/Get Status of Group Elements'), [varXpath:prefsXpath, varList: '',
				varCheckedOnly: false], FailureHandling.STOP_ON_FAILURE)
			
			prefs = []
			
			ministryPrefs.each {
				println(it.value[0])
				prefs.add(it.value[0])
			}
			
			selections.put('Ministry Preferences', prefs)
			...
			if ((line != null) && line.contains(':')) {
				...
				while (isField) {
					...
					if (keyFound || field == 'Contact Name' || field == 'Contact Email' || field == 'Alternate Job Title' || field == 'IT Job Category' || field == 'Need to See Specific IT Jobs?') {
						...
						if (lastValue != 'Group' || field.contains('IT Job Category') || field.contains('Need to See Specific IT Jobs?')) {
