...
//					if (keyFound) {
					if (keyFound || field == 'Contact Name' || field == 'Contact Email' || field == 'Alternate Job Title' || field == 'IT Job Category') {
						lastValue = values[values.size() - 1]
						if (lastValue != 'Group') { // && !lastValue.contains('IT Job Category')) {
							for (def value : values) {
								if (ignoreRows.contains(value)) { //&& value != 'IT Job Category') {
									println(('removing ' + value) + ' from values')
