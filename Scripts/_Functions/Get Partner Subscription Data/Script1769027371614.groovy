...
if (row_count > 0) {
    for (row = 1; row < row_count; row++) {
		...
		if(line.contains('Expired Accounts:')) {
			acctType = 'expired'
			continue
		}
		
		if(line.contains('Test/Demo Accounts:')) {
			acctType = 'test'
			continue
		}
		...
		if(acctType == 'active' || (acctType == 'test' && includeTestAccts)) {
			...
			values.addAll(organization, start, expires, username, acctType)
